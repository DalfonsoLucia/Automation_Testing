...
test.only('Web Client App Login, add product to Cart and make a Order', async ({page})=>
{
...
test('Search ID Order in My Orders Tab', async ({page})=>
{
    ...
    await rows.filter(page.getByRole("row")).first().getByRole("button", {name: "View"}).click();
