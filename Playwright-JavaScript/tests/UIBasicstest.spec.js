...
test('@Web Login with right credentials and first element found', async ({browser})=>
    {
        const context = await browser.newContext(); //si istanzia per creare un nuovo contesto
        const page = await context.newPage(); //per creare una nuova pagina
...
test('Login with right credentials and select item in a static dropdown and select radiobutton and checkbox', async ({page})=>
{
...
test('Using async await with Assertions and understand validating the attributes', async ({page})=>
{
    const username = page.locator('#username');
    const password = page.locator("[type='password']");
    const signIn = page.locator("#signInBtn");
...
test('Codegen tool to record & Playback with generated automation script', async ({page})=>
{
